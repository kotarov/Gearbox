            ...
            if(ret.data && ret.data[0]) $.each(ret.data[0], function(k,v){
                modal.find("[name='"+k+"']").each(function(r,input){ input = $(this);
                    if(input.prop("tagName") == "SELECT"){
                        ...
                        }else if( input.attr("data-tinymce") !== "undefined" && typeof $(input).attr("id")=="string"){
...
$("form").not(".no-ajax").submit(function(e){ 
    ...
    $.ajax({
        ...
        if(ret.required){
            ...
            $form.prepend('<div class="uk-alert uk-alert-danger"><b>'+(window['lang'][msg]||msg)+'</b></div>').scrollTop();
        }
        if(ret.success){
